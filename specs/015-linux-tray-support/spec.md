...
- **FR-015**: *(Deferred — v2)* The "Start at login" preference SHOULD eventually create/remove an XDG autostart entry for the Adagio desktop app (`~/.config/autostart/adagio.desktop`) on Linux. This is distinct from the pre-existing `adagio-daemon.desktop` managed by the daemon. In v1, the daemon's "Start at login" feature (already implemented) is the only autostart mechanism; the GUI launches manually. See `research.md` Finding 7 for rationale.
